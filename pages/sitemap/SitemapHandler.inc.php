		...
		if ($journal->getData('publishingMode') != PUBLISHING_MODE_NONE) {
			...
			while ($issue = $publishedIssues->next()) {
				...
				foreach($submissions as $submission) {
					// Abstract
					$root->appendChild($this->_createUrlTree($doc, $request->url($journal->getPath(), 'article', 'view', array($submission->getBestArticleId()))));
					// Galley files
					$galleys = $galleyDao->getByPublicationId($submission->getCurrentPublication()->getId());
					while ($galley = $galleys->next()) {
						$root->appendChild($this->_createUrlTree($doc, $request->url($journal->getPath(), 'article', 'view', array($submission->getBestArticleId(), $galley->getBestGalleyId()))));
					}
				}
			}
		}
