			...
			case 'ynp': // Volume, number, year, page
				...
				foreach ($submissions as $submission) {
					...
					if (PKPString::regexp_match_get('/^[Pp][Pp]?[.]?[ ]?(\d+)$/', $submission->getPages(), $matches)) {
						...
						if ($page == $matchedPage) $request->redirect(null, 'article', 'view', $submission->getBestArticleId());
					}
					if (PKPString::regexp_match_get('/^[Pp][Pp]?[.]?[ ]?(\d+)[ ]?-[ ]?([Pp][Pp]?[.]?[ ]?)?(\d+)$/', $submission->getPages(), $matches)) {
						...
						if ($page >= $matchedPageFrom && ($page < $matchedPageTo || ($page == $matchedPageTo && $matchedPageFrom = $matchedPageTo))) $request->redirect(null, 'article', 'view', $submission->getBestArticleId());
					}
					unset($submission);
				}
				break;
